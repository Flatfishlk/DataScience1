...
def array_work(rows, cols, scalar, matrixA):
    ...
    Ex: array_work(2, 3, 5, [[3, 4], [5, 6], [7, 8]])
           [[3, 4],      [[5, 5, 5],
            ...
            [7, 8]]
    '''
    return np.dot(np.full((rows,cols),scalar), matrixA)
# rows = 2
# cols = 2
# scalar = 5
# matrixA = [[1,2], [2,3]]
# print array_work(rows, cols, scalar, matrixA)
...
def markets_per_state():
    ...
    '''
    return "select State, count(FMID) from farmersmarkets where WIC = Y or WICcash = Y group by State"
